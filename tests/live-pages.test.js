...
test("canlı Pages denetimi Vite ve eski çalışma zamanı paketini doğrular",async()=>{
  ...
  assert.deepEqual(verifyLiveAssets({index,bundle:"YKS_V4_RELEASE_OK 4.0.0 stable",legacyApp:'const APP_VERSION="3.2.7"'}),{bundlePath:"assets/index-AbC123.js",release:"4.0.0",legacyVersion:"3.2.7"});
});
...
test("canlı Pages denetimi kaynak TypeScript veya bozuk UTF-8 paketini reddeder",async()=>{
  ...
  assert.throws(()=>verifyLiveAssets({index:`<script type="module" src="./src/main.ts"></script><script type="module" src="./assets/index-x.js"></script>${legacy}`,bundle:"YKS_V4_RELEASE_OK 4.0.0 stable",legacyApp:'const APP_VERSION="3.2.7"'}),/TypeScript kaynak/);
  assert.throws(()=>verifyLiveAssets({index:`<script type="module" src="./assets/index-x.js"></script>${legacy}`,bundle:"YKS_V4_RELEASE_OK 4.0.0 stable",legacyApp:'const APP_VERSION="3.2.7";/*�*/'}),/UTF-8/);
});
