...
test("canlı Pages denetimi Vite ve eski çalışma zamanı paketini doğrular",async()=>{
  ...
  assert.deepEqual(verifyLiveAssets({index,bundle:"YKS_V4_RELEASE_OK 4.1.0 stable",legacyApp:'const APP_VERSION="4.1.0";const APP_BUILD="4.1.0-r20"'}),{bundlePath:"assets/index-AbC123.js",release:"4.1.0",legacyVersion:"4.1.0",build:"4.1.0-r20"});
});
...
test("canlı Pages denetimi kaynak TypeScript veya bozuk UTF-8 paketini reddeder",async()=>{
  ...
  assert.throws(()=>verifyLiveAssets({index:`<script type="module" src="./src/main.ts"></script><script type="module" src="./assets/index-x.js"></script>${legacy}`,bundle:"YKS_V4_RELEASE_OK 4.1.0 stable",legacyApp:app}),/TypeScript kaynak/);
  assert.throws(()=>verifyLiveAssets({index:`<script type="module" src="./assets/index-x.js"></script>${legacy}`,bundle:"YKS_V4_RELEASE_OK 4.1.0 stable",legacyApp:app+'/*�*/'}),/UTF-8/);
});
