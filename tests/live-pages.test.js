...
test("canlı Pages denetimi gerçek release sürümünü, güvenli runtime'ı ve eski çalışma zamanı paketini doğrular",async()=>{
  ...
  assert.deepEqual(verifyPolicyPages({privacy:"YKS Defterim Gizlilik Politikası IndexedDB/Dexie",deletion:"YKS Defterim cihaz verilerini silme Cihaz verilerini sil IndexedDB/Dexie"}),{privacy:true,deletion:true});
});
...
test("canlı Play politika sayfaları eksik veya eski davranışta başarısız olur",async()=>{
  ...
  assert.throws(()=>verifyPolicyPages({privacy:"YKS Defterim Gizlilik Politikası IndexedDB\/Dexie Firebase",deletion:"YKS Defterim cihaz verilerini silme Cihaz verilerini sil IndexedDB\/Dexie"}),/uyumsuz/);
});
