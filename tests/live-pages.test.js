...
const goodBundle="YKS_V4_RELEASE_OK 4.3.1 stable v43Runtime v43RuntimeErrors degraded v43FocusSessionGuard";
const goodSw='const APP_VERSION="4.3.1";const APP_BUILD="4.3.1-r1";const CACHE="yks-core-v4.3.1-r1"';
...
test("canlı Pages denetimi gerçek release sürümünü, güvenli runtime'ı ve eski çalışma zamanı paketini doğrular",async()=>{
  ...
  assert.deepEqual(verifyLiveAssets({index,bundle:goodBundle,legacyApp:'const APP_VERSION="4.1.0";const APP_BUILD="4.1.0-r20"',serviceWorker:goodSw}),{bundlePath:"assets/index-AbC123.js",release:"4.3.1",releaseBuild:"4.3.1-r1",legacyVersion:"4.1.0",legacyBuild:"4.1.0-r20"});
});
...
test("canlı Pages denetimi yanlış release, kaynak TypeScript, bozuk UTF-8 veya yanlış PWA cache'ini reddeder",async()=>{
  ...
  assert.throws(()=>verifyLiveAssets({index,bundle:goodBundle.replace("4.3.1","4.3.0"),legacyApp:app,serviceWorker:goodSw}),/4\.3\.1/);
  ...
  assert.throws(()=>verifyLiveAssets({index,bundle:goodBundle,legacyApp:app,serviceWorker:goodSw.replace("4.3.1-r1","4.3.0-r1")}),/service worker/);
});
