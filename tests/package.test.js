...
test("v4.4.0 kararlı sürümü tarayıcı ve üretim paketi kontrollerini içerir",()=>{
  ...
  assert.match(version,/RELEASE_VERSION="4\.4\.0"/);assert.match(version,/RELEASE_BUILD="4\.4\.0-r1"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);assert.match(release,/RELEASE_VERSION/);assert.match(release,/SCREEN_IDS/);assert.match(release,/release-overlay/);assert.match(release,/backup-recovery/);assert.match(release,/pwa-build/);assert.match(release,/v43-runtime/);assert.match(release,/dexie-write-through/);assert.match(release,/firebase-payload/);assert.match(release,/firebase-download-path/);assert.match(release,/screen-transitions/);assert.match(release,/YKS_V4_RELEASE_OK/);assert.match(entry,/installReleaseRuntime\(\)/);assert.match(entry,/RELEASE_CHANNEL/);assert.match(entry,/stableRelease:true/);assert.match(entry,/installReleaseOverlay\(\)/);assert.match(entry,/installV43SafeRuntime\(\)/);assert.match(overlay,/remoteVersionIsNewer/);assert.match(legacy,/v4-bootstrap/);assert.match(verify,/runTransaction as/);assert.match(report,/\?selftest=v4/);
});
