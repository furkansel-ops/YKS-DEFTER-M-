...
test("sürüm, şema ve PWA önbelleği tutarlı",()=>{
  ...
  assert.equal(version.version,"4.3.0");assert.equal(version.schema,21);assert.equal(version.build,"4.3.0-r1");assert.match(releaseVersion,/RELEASE_VERSION="4\.3\.0"/);assert.match(releaseVersion,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);
  assert.match(sw,/const APP_VERSION="4\.3\.0"/);assert.match(sw,/const APP_BUILD="4\.3\.0-r1"/);assert.match(sw,/const CACHE="yks-core-v4\.3\.0-r1"/);assert.match(sw,/yks-core-v4\.2\.0-r1/);
...
test("v4 PWA çevrimdışı paketi atomik hazırlanır ve yapı numarasıyla güncellenir",()=>{
  ...
  assert.match(sw,/function buildAssets/);assert.match(sw,/matchAll/);assert.match(sw,/Promise\.all\(required/);assert.match(sw,/await caches\.delete\(CACHE\)/);assert.match(sw,/GET_CACHE_STATUS/);assert.match(sw,/READY_KEY/);assert.match(sw,/const APP_BUILD="4\.3\.0-r1"/);
...
test("v4.3 kararlı sürümü tarayıcı ve üretim paketi kontrollerini içerir",()=>{
  ...
  assert.match(version,/RELEASE_VERSION="4\.3\.0"/);assert.match(version,/RELEASE_BUILD="4\.3\.0-r1"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);
...
test("v4.3.0 kararlı sürümü geçici aday dosyası bırakmaz ve veri şemasını korur",()=>{
  ...
  assert.equal(pkg.version,"4.3.0");assert.equal(lock.lockfileVersion,3);assert.equal(lock.packages[""].name,"yks-defterim");assert.deepEqual(lock.packages[""].dependencies,pkg.dependencies);assert.deepEqual(lock.packages[""].devDependencies,pkg.devDependencies);
  ...
  assert.match(entry,/version:RELEASE_VERSION/);assert.match(entry,/channel:RELEASE_CHANNEL/);assert.match(version,/RELEASE_VERSION="4\.3\.0"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);assert.match(app,/const DATA_SCHEMA=21/);
});
