...
test("sürüm, şema ve PWA önbelleği tutarlı",()=>{
  const html=fs.readFileSync(path.join(root,"index.html"),"utf8"),app=fs.readFileSync(path.join(root,"app.js"),"utf8"),sw=fs.readFileSync(path.join(root,"sw.js"),"utf8"),version=JSON.parse(fs.readFileSync(path.join(root,"version.json"),"utf8")),releaseVersion=fs.readFileSync(path.join(root,"src/release/version.ts"),"utf8");
  ...
  assert.equal(version.version,"4.2.0");assert.equal(version.schema,21);assert.equal(version.build,"4.2.0-r1");assert.match(releaseVersion,/RELEASE_VERSION="4\.2\.0"/);assert.match(releaseVersion,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);
  assert.match(sw,/const APP_VERSION="4\.2\.0"/);assert.match(sw,/const APP_BUILD="4\.2\.0-r1"/);assert.match(sw,/const CACHE="yks-core-v4\.2\.0-r1"/);assert.match(sw,/yks-core-v4\.1\.0-r40/);
  ...
  assert.match(html,/src="\.\/modules\/learning-lab\.js\?v=4\.1\.0-r26"/);
...
test("v4 Vite ve TypeScript geçiş altyapısı güvenli biçimde hazır",()=>{
  ...
  assert.match(html,/type="module" src="\.\/src\/main\.ts"/);assert.match(entry,/legacyRuntime:true/);assert.match(entry,/RELEASE_VERSION/);assert.match(entry,/RELEASE_BUILD/);assert.match(copy,/"modules"/);assert.equal(pkg.devDependencies.dexie,undefined);
});
...
test("v4 güvenli yedek köprüsü imza, önizleme ve Dexie geri yükleme akışını paketler",()=>{
  ...
  assert.match(source,/BACKUP_FORMAT_VERSION=3/);assert.match(source,/fnv1a-32/);assert.match(source,/inspectBackupPackage/);assert.match(source,/previewBackupPackage/);assert.match(source,/applyBackupJSON/);assert.match(source,/rolledBack/);
  assert.match(entry,/installLegacyBackupBridge/);assert.match(entry,/installRecoveryCenter/);assert.match(app,/Yedek doğrulandı/);assert.match(app,/AUTO_BACKUP_GUN=14/);assert.match(html,/Güvenli yedekleme etkin/);assert.match(html,/Yedeği kontrol et ve geri yükle/);
});
...
test("v4 PWA çevrimdışı paketi atomik hazırlanır ve yapı numarasıyla güncellenir",()=>{
  ...
  assert.match(sw,/function buildAssets/);assert.match(sw,/matchAll/);assert.match(sw,/Promise\.all\(required/);assert.match(sw,/await caches\.delete\(CACHE\)/);assert.match(sw,/GET_CACHE_STATUS/);assert.match(sw,/READY_KEY/);assert.match(sw,/const APP_BUILD="4\.2\.0-r1"/);
  ...
  assert.equal(manifest.prefer_related_applications,false);assert.deepEqual(manifest.display_override,["window-controls-overlay","standalone"]);assert.match(entry,/installPwaRuntime\(RELEASE_BUILD\)/);assert.match(pwa,/beforeinstallprompt/);assert.match(pwa,/Ana ekrana ekle/);
});
...
test("v4.2 kararlı sürümü tarayıcı ve üretim paketi kontrollerini içerir",()=>{
  const release=fs.readFileSync(path.join(root,"src/release/release.ts"),"utf8"),entry=fs.readFileSync(path.join(root,"src/main.ts"),"utf8"),version=fs.readFileSync(path.join(root,"src/release/version.ts"),"utf8"),overlay=fs.readFileSync(path.join(root,"src/release/release-overlay.ts"),"utf8"),legacy=fs.readFileSync(path.join(root,"modules/release-selftest.js"),"utf8"),verify=fs.readFileSync(path.join(root,"scripts/verify-release.mjs"),"utf8"),report=fs.readFileSync(path.join(root,"RELEASE.md"),"utf8");
  assert.match(version,/RELEASE_VERSION="4\.2\.0"/);assert.match(version,/RELEASE_BUILD="4\.2\.0-r1"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);
  assert.match(release,/RELEASE_VERSION/);assert.match(release,/SCREEN_IDS/);assert.match(release,/release-overlay/);assert.match(release,/backup-recovery/);assert.match(release,/pwa-build/);assert.match(release,/dexie-write-through/);assert.match(release,/firebase-payload/);assert.match(release,/firebase-download-path/);assert.match(release,/screen-transitions/);assert.match(release,/YKS_V4_RELEASE_OK/);
  assert.match(entry,/installReleaseRuntime\(\)/);assert.match(entry,/RELEASE_CHANNEL/);assert.match(entry,/stableRelease:true/);assert.match(entry,/installReleaseOverlay\(\)/);assert.match(overlay,/remoteVersionIsNewer/);assert.match(legacy,/v4-bootstrap/);assert.match(verify,/runTransaction as/);assert.match(report,/\?selftest=v4/);
});
...
test("v4.2.0 kararlı sürümü geçici aday dosyası bırakmaz ve veri şemasını korur",()=>{
  const pkg=JSON.parse(fs.readFileSync(path.join(root,"package.json"),"utf8")),lock=JSON.parse(fs.readFileSync(path.join(root,"package-lock.json"),"utf8")),entry=fs.readFileSync(path.join(root,"src/main.ts"),"utf8"),app=fs.readFileSync(path.join(root,"app.js"),"utf8"),version=fs.readFileSync(path.join(root,"src/release/version.ts"),"utf8");
  assert.equal(pkg.version,"4.2.0");assert.equal(lock.lockfileVersion,3);assert.equal(lock.packages[""].name,"yks-defterim");assert.deepEqual(lock.packages[""].dependencies,pkg.dependencies);assert.deepEqual(lock.packages[""].devDependencies,pkg.devDependencies);
  ...
  ["src/release/version.ts","src/release/release-overlay.ts","src/release/release.ts","scripts/verify-release.mjs","tests/release.test.js","RELEASE.md"].forEach(file=>assert.equal(fs.existsSync(path.join(root,file)),true,file));
  assert.match(entry,/version:RELEASE_VERSION/);assert.match(entry,/channel:RELEASE_CHANNEL/);assert.match(version,/RELEASE_VERSION="4\.2\.0"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);assert.match(app,/const DATA_SCHEMA=21/);
});
