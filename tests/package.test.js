...
test("sürüm, şema ve PWA önbelleği tutarlı",()=>{
  ...
  assert.match(html,/src="\.\/app\.js\?v=4\.4\.0-r4"/);assert.match(html,/src="\.\/modules\/stability\.js\?v=4\.1\.0-r28"/);assert.match(app,/const APP_VERSION="4\.1\.0"/);assert.match(app,/const APP_BUILD="4\.1\.0-r20"/);assert.match(app,/const DATA_SCHEMA=21/);
  assert.equal(version.version,"4.4.0");assert.equal(version.schema,21);assert.equal(version.build,"4.4.0-r4");assert.match(releaseVersion,/RELEASE_VERSION="4\.4\.0"/);assert.match(releaseVersion,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);
...
test("v4 PWA çevrimdışı paketi atomik hazırlanır ve yapı numarasıyla güncellenir",()=>{
  ...
  ["v4PwaCard","v4PwaBadge","v4InstallBtn","v4OfflineState"].forEach(id=>assert.match(html,new RegExp(`id="${id}"`),id));assert.match(sw,/function buildAssets/);assert.match(sw,/matchAll/);assert.match(sw,/Promise\.all\(required/);assert.match(sw,/await caches\.delete\(CACHE\)/);assert.match(sw,/GET_CACHE_STATUS/);assert.match(sw,/READY_KEY/);assert.match(sw,/const APP_BUILD="4\.4\.0-r4"/);assert.match(sw,/yks-core-v4\.4\.0-r1/);assert.match(sw,/yks-core-v4\.3\.1-r1/);assert.match(sw,/yks-core-v4\.3\.0-r1/);assert.match(sw,/yks-core-v4\.2\.0-r1/);assert.doesNotMatch(sw,/cache\.put\("\.\/index\.html",res\.clone\(\)\)/);assert.match(app,/j\.build\|\|j\.version/);assert.match(app,/await window\.__YKS_DATA__\.flush\(\)/);assert.match(app,/!appUpdateApplying\|\|appUpdateReloading/);assert.equal(manifest.prefer_related_applications,false);assert.deepEqual(manifest.display_override,["window-controls-overlay","standalone"]);assert.match(entry,/installPwaRuntime\(RELEASE_BUILD\)/);assert.match(pwa,/beforeinstallprompt/);assert.match(pwa,/Ana ekrana ekle/);
});
...
test("v4.4.0 kararlı sürümü tarayıcı ve üretim paketi kontrollerini içerir",()=>{
  ...
  assert.match(version,/RELEASE_VERSION="4\.4\.0"/);assert.match(version,/RELEASE_BUILD="4\.4\.0-r4"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);assert.match(release,/RELEASE_VERSION/);assert.match(release,/SCREEN_IDS/);assert.match(release,/release-overlay/);assert.match(release,/backup-recovery/);assert.match(release,/pwa-build/);assert.match(release,/v43-runtime/);assert.match(release,/dexie-write-through/);assert.match(release,/portable-payload/);assert.match(release,/external-restore-path/);assert.match(release,/screen-transitions/);assert.match(release,/YKS_V4_RELEASE_OK/);assert.match(entry,/installReleaseRuntime\(\)/);assert.match(entry,/stableRelease:true/);assert.match(entry,/channel:RELEASE_CHANNEL/);assert.match(entry,/installReleaseOverlay\(\)/);assert.match(entry,/installV43SafeRuntime\(\)/);assert.match(overlay,/remoteVersionIsNewer/);assert.match(legacy,/v4-bootstrap/);assert.match(verify,/bundledOptionalCloud/);assert.match(report,/\?selftest=v4/);
});
