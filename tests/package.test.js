...
test("sürüm, şema ve PWA önbelleği tutarlı",()=>{
  ...
  assert.equal(version.version,"4.4.0");assert.equal(version.schema,21);assert.equal(version.build,"4.4.0-r2");assert.match(releaseVersion,/RELEASE_VERSION="4\.4\.0"/);assert.match(releaseVersion,/RELEASE_BUILD="4\.4\.0-r2"/);assert.match(releaseVersion,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);
  assert.match(sw,/const APP_VERSION="4\.4\.0"/);assert.match(sw,/const APP_BUILD="4\.4\.0-r2"/);assert.match(sw,/const CACHE="yks-core-v4\.4\.0-r2"/);assert.match(sw,/yks-core-v4\.4\.0-r1/);assert.match(sw,/yks-core-v4\.3\.1-r1/);assert.match(sw,/yks-core-v4\.3\.0-r1/);assert.match(sw,/yks-core-v4\.2\.0-r1/);assert.match(sw,/yks-core-v4\.1\.0-r40/);
...
test("v4 PWA çevrimdışı paketi atomik hazırlanır ve yapı numarasıyla güncellenir",()=>{
  ...
  ["v4PwaCard","v4PwaBadge","v4InstallBtn","v4OfflineState"].forEach(id=>assert.match(html,new RegExp(`id="${id}"`),id));assert.match(sw,/function buildAssets/);assert.match(sw,/matchAll/);assert.match(sw,/cacheAssetBatch\(cache,required\)/);assert.doesNotMatch(sw,/await caches\.delete\(CACHE\)/);assert.match(sw,/GET_CACHE_STATUS/);assert.match(sw,/READY_KEY/);assert.match(sw,/const APP_BUILD="4\.4\.0-r2"/);assert.match(sw,/const CACHE="yks-core-v4\.4\.0-r2"/);assert.match(sw,/yks-core-v4\.4\.0-r1/);assert.match(sw,/yks-core-v4\.3\.1-r1/);assert.match(sw,/yks-core-v4\.3\.0-r1/);assert.match(sw,/yks-core-v4\.2\.0-r1/);assert.doesNotMatch(sw,/cache\.put\("\.\/index\.html",res\.clone\(\)\)/);assert.match(app,/j\.build\|\|j\.version/);assert.match(app,/await window\.__YKS_DATA__\.flush\(\)/);assert.match(app,/!appUpdateApplying\|\|appUpdateReloading/);assert.equal(manifest.prefer_related_applications,false);assert.deepEqual(manifest.display_override,["window-controls-overlay","standalone"]);assert.match(entry,/installPwaRuntime\(RELEASE_BUILD\)/);assert.match(pwa,/beforeinstallprompt/);assert.match(pwa,/Ana ekrana ekle/);
});
...
test("v4.4.0 kararlı sürümü tarayıcı ve üretim paketi kontrollerini içerir",()=>{
  ...
  assert.match(version,/RELEASE_VERSION="4\.4\.0"/);assert.match(version,/RELEASE_BUILD="4\.4\.0-r2"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);assert.match(release,/RELEASE_VERSION/);assert.match(release,/SCREEN_IDS/);assert.match(release,/release-overlay/);assert.match(release,/backup-recovery/);assert.match(release,/pwa-build/);assert.match(release,/v43-runtime/);assert.match(release,/dexie-write-through/);assert.match(release,/portable-payload/);assert.match(release,/external-restore-path/);assert.match(release,/screen-transitions/);assert.match(release,/YKS_V4_RELEASE_OK/);assert.match(entry,/installReleaseRuntime\(\)/);assert.match(entry,/RELEASE_CHANNEL/);assert.match(entry,/stableRelease:true/);assert.match(entry,/installReleaseOverlay\(\)/);assert.match(entry,/installV43SafeRuntime\(\)/);assert.match(overlay,/remoteVersionIsNewer/);assert.match(legacy,/v4-bootstrap/);assert.match(verify,/localOnly/);assert.match(report,/\?selftest=v4/);
});

test("CI ve Pages dağıtımı güncel eylemler, zaman aşımı ve canlı doğrulama kullanır",()=>{
  const ci=read(".github/workflows/ci.yml"),deploy=read(".github/workflows/deploy-pages.yml"),live=read("scripts/verify-live-pages.mjs");assert.match(ci,/pull_request:/);assert.match(ci,/cancel-in-progress: true/);assert.match(ci,/timeout-minutes: 10/);assert.match(ci,/npm run build:assets/);assert.match(ci,/node scripts\/verify-release\.mjs/);assert.match(ci,/node scripts\/verify-v440-production\.mjs/);assert.doesNotMatch(ci,/npm run release:check/);[ci,deploy].forEach(workflow=>{assert.match(workflow,/actions\/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1/);assert.match(workflow,/actions\/setup-node@820762786026740c76f36085b0efc47a31fe5020/);});assert.match(deploy,/actions\/configure-pages@45bfe0192ca1faeb007ade9deae92b16b8254a0d/);assert.match(deploy,/actions\/upload-pages-artifact@fc324d3547104276b827a68afc52ff2a11cc49c9/);assert.match(deploy,/actions\/deploy-pages@cd2ce8fcbc39b97be8ca5fce6e763baed58fa128/);for(const file of fs.readdirSync(path.join(root,".github/workflows")).filter(file=>file.endsWith(".yml")))assert.doesNotMatch(read(`.github/workflows/${file}`),/uses:\s+[^\s#]+@v\d+/);assert.match(deploy,/conclusion/);assert.match(deploy,/node scripts\/verify-live-pages\.mjs/);assert.match(live,/verifyLivePages/);assert.match(live,/YKS_V4_RELEASE_OK/);assert.match(live,/AbortSignal\.timeout/);
});
