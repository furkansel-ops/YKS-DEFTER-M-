...
test("sürüm, şema ve PWA önbelleği tutarlı",()=>{
  ...
  assert.equal(version.version,"4.3.1");assert.equal(version.schema,21);assert.equal(version.build,"4.3.1-r1");assert.match(releaseVersion,/RELEASE_VERSION="4\.3\.1"/);assert.match(releaseVersion,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);
  assert.match(sw,/const APP_VERSION="4\.3\.1"/);assert.match(sw,/const APP_BUILD="4\.3\.1-r1"/);assert.match(sw,/const CACHE="yks-core-v4\.3\.1-r1"/);assert.match(sw,/yks-core-v4\.3\.0-r1/);assert.match(sw,/yks-core-v4\.2\.0-r1/);assert.match(sw,/yks-core-v4\.1\.0-r40/);
...
test("dağıtım paketinde gömülü YouTube anahtarı yok",()=>{
  ...
  assert.doesNotMatch(all,/AIzaSyALxmrdmd2UnoZcxN7HjWNKeS3g7B_o9LU/);assert.match(all,/const YT_BUILTIN_KEY=""/);
});
...
test("1000 sözün İnsan Sözü bölümünde İngilizce kayıt kalmadı",()=>{
  const vm=require("node:vm"),app=read("app.js"),match=app.match(/const SOZLER=(\[[\s\S]*?\]);\ntry\{ window\.SOZLER/);assert.ok(match);
...
test("v4 Firebase senkronu Dexie ana kayıt katmanını kullanır",()=>{
  const cloud=read("src/data/cloud-state.ts"),primary=read("src/data/primary-store.ts"),bridge=read("src/data/legacy-data-bridge.ts"),html=read("index.html");
  assert.match(cloud,/buildCloudPayload/);assert.match(cloud,/state\.focus/);assert.match(cloud,/state\.yt/);assert.match(cloud,/stateHash\(json\)/);assert.match(primary,/replaceFromExternal/);assert.match(primary,/source:Extract<StateWriteSource,"firebase"\|"backup">="firebase"/);assert.match(primary,/persistJSON\(json,updatedAt,source\)/);assert.match(primary,/readPrimaryJSON/);assert.match(bridge,/cloudPayload/);assert.match(bridge,/applyCloudJSON/);assert.match(bridge,/await flush\(\)/);assert.match(html,/async function cloudJSON/);assert.match(html,/await window\.__YKS_DATA__\.cloudPayload\(\)/);assert.match(html,/await window\.__YKS_DATA__\.applyCloudJSON\(persisted\)/);assert.match(html,/const json=await cloudJSON\(\)/);assert.match(html,/runTransaction\(db/);assert.match(html,/SYNC_CONFLICT/);assert.match(html,/infraHash\(json\)/);
});
...
test("v4.3.1 kararlı sürümü tarayıcı ve üretim paketi kontrollerini içerir",()=>{
  ...
  assert.match(version,/RELEASE_VERSION="4\.3\.1"/);assert.match(version,/RELEASE_BUILD="4\.3\.1-r1"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);assert.match(release,/RELEASE_VERSION/);assert.match(release,/SCREEN_IDS/);assert.match(release,/release-overlay/);assert.match(release,/backup-recovery/);assert.match(release,/pwa-build/);assert.match(release,/v43-runtime/);assert.match(release,/dexie-write-through/);assert.match(release,/firebase-payload/);assert.match(release,/firebase-download-path/);assert.match(release,/screen-transitions/);assert.match(release,/YKS_V4_RELEASE_OK/);assert.match(entry,/installReleaseRuntime\(\)/);assert.match(entry,/RELEASE_CHANNEL/);assert.match(entry,/stableRelease:true/);assert.match(entry,/installReleaseOverlay\(\)/);assert.match(entry,/installV43SafeRuntime\(\)/);assert.match(overlay,/remoteVersionIsNewer/);assert.match(legacy,/v4-bootstrap/);assert.match(verify,/runTransaction as/);assert.match(report,/\?selftest=v4/);
});

test("CI ve Pages dağıtımı güncel eylemler, zaman aşımı ve canlı doğrulama kullanır",()=>{
  const ci=read(".github/workflows/ci.yml"),deploy=read(".github/workflows/deploy-pages.yml"),live=read("scripts/verify-live-pages.mjs");assert.match(ci,/pull_request:/);assert.match(ci,/cancel-in-progress: true/);assert.match(ci,/timeout-minutes: 10/);assert.match(ci,/npm run release:check/);[ci,deploy].forEach(workflow=>{assert.match(workflow,/actions\/checkout@v7/);assert.match(workflow,/actions\/setup-node@v7/);});assert.match(deploy,/actions\/configure-pages@v6/);assert.match(deploy,/actions\/upload-pages-artifact@v5/);assert.match(deploy,/actions\/deploy-pages@v5/);assert.match(deploy,/conclusion/);assert.match(deploy,/node scripts\/verify-live-pages\.mjs/);assert.match(live,/verifyLivePages/);assert.match(live,/YKS_V4_RELEASE_OK/);assert.match(live,/AbortSignal\.timeout/);
});

test("v4.3.1 kararlı sürümü geçici aday dosyası bırakmaz ve veri şemasını korur",()=>{
  ...
  assert.equal(pkg.version,"4.3.1");assert.equal(lock.lockfileVersion,3);assert.equal(lock.packages[""].name,"yks-defterim");assert.deepEqual(lock.packages[""].dependencies,pkg.dependencies);assert.deepEqual(lock.packages[""].devDependencies,pkg.devDependencies);
  ["src/release/release-candidate.ts","scripts/verify-release-candidate.mjs","tests/release-candidate.test.js","RELEASE-CANDIDATE.md"].forEach(file=>assert.equal(fs.existsSync(path.join(root,file)),false,file));["src/release/version.ts","src/release/release-overlay.ts","src/release/release.ts","scripts/verify-release.mjs","tests/release.test.js","RELEASE.md"].forEach(file=>assert.equal(fs.existsSync(path.join(root,file)),true,file));assert.match(entry,/version:RELEASE_VERSION/);assert.match(entry,/channel:RELEASE_CHANNEL/);assert.match(version,/RELEASE_VERSION="4\.3\.1"/);assert.match(version,/LEGACY_CORE_BUILD="4\.1\.0-r20"/);assert.match(app,/const DATA_SCHEMA=21/);
});
