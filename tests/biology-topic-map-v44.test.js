...
test("v4.4 konu haritası yalnız Adım adım görünümünde açılır, mevcut durak seçimini kullanır ve quiz cevabını sızdırmaz",()=>{
  ...
  assert.match(lessons,/atlasTopicMapV44\(t,step\)/);assert.match(lessons,/focused=view==="steps"/);assert.match(ui,/data-atlas-step/);assert.match(ui,/aria-current=\\"step\\"/);assert.match(ui,/AYT GÖRSEL KONU HARİTASI/);assert.match(ui,/YKS ANA FİKİR/);assert.match(ui,/KARIŞTIRMA NOKTASI/);assert.doesNotMatch(ui,/quiz\.answer|answerAtlas|picked/);assert.match(css,/pointer:coarse/);assert.match(css,/prefers-reduced-motion:reduce/);
