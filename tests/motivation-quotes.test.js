...
test('Günün sözü yalnızca teknik direktör ve futbolcu havuzlarıyla sınırlıdır',()=>{
  let n=0;const {context,box}=runWithRandom(max=>(n++*37)%max);
  const meta=context.window.__YKS_MOTIVATION_QUOTES_READY__;
  assert.equal(meta.scope,'football-only');assert.equal(meta.style,'v2');assert.equal(meta.version,'2.4.0');
  assert.ok(meta.coachPool>=10);assert.ok(meta.playerPool>=8);assert.equal('examPool' in meta,false);
  ...
  let sawCoach=false,sawPlayer=false;
  for(let i=0;i<500;i++){
  ...
  assert.equal(sawCoach,true);assert.equal(sawPlayer,true);
});

test('futbol havuzu yalnızca teknik direktörler ve futbolculardan oluşur',()=>{
  const source=fs.readFileSync(path.join(root,'modules/motivation-quotes-v1.js'),'utf8');
  assert.doesNotMatch(source,/const EXAM_QUOTES=/);assert.match(source,/const COACH_QUOTES=/);assert.match(source,/const PLAYER_QUOTES=/);
...
test('eski app.js fallbackı genel insan sözlerini ekrana basmaz',()=>{
  ...
  assert.match(v319Block,/soz\.c\|\|"Futbol"/);
