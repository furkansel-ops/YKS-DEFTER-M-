...
function harness(){
  ...
  const document={documentElement:{dataset:{v42ReleaseOverlay:"ready",appVersion:"4.2.0",v4RecoveryErrors:"0"}},body:{appendChild:node=>{output=node;}},querySelectorAll:selector=>selector===".screen.active"?screens.filter(x=>x.id===active):selector===".screen"?screens:[],getElementById:id=>id==="v4ReleaseResult"?output:null,createElement:()=>({id:"",hidden:false,textContent:""})};
...
test("kararlı sürüm denetimi açılış, ekran, Dexie, kurtarma ve bulut yolunu birlikte doğrular",async()=>{
  ...
  try{
    ...
    assert.equal(result.ok,true);assert.equal(result.version,"4.2.0");assert.ok(result.checks.length>=20);assert.equal(result.checks.every(x=>x.ok),true);assert.equal(h.active,"home");assert.equal(h.applied,2);assert.match(h.output.textContent,/YKS_V4_RELEASE_OK/);assert.equal(api.latest(),result);
    for(const name of ["bootstrap","release-overlay","backup-recovery","pwa-build","dexie-write-through","firebase-payload","screen-transitions"])assert.equal(result.checks.find(x=>x.name===name)?.ok,true,name);
  }finally{if(previous===undefined)delete global.CustomEvent;else global.CustomEvent=previous;}
});

test("zorunlu köprü hatası kararlı sürümü başarısız işaretler",async()=>{
  const previous=global.CustomEvent;global.CustomEvent=class{constructor(type,options){this.type=type;this.detail=options&&options.detail;}};
  try{
    const {createReleaseRuntime}=await import(releaseUrl),h=harness();h.host.__YKS_DOMAIN__.validate=()=>["hata"];
    const result=await createReleaseRuntime(h.host,h.document).run({exerciseScreens:false,persistRoundTrip:false});assert.equal(result.ok,false);assert.equal(result.checks.find(x=>x.name==="domain-services").ok,false);assert.match(h.output.textContent,/YKS_V4_RELEASE_FAIL/);
  }finally{if(previous===undefined)delete global.CustomEvent;else global.CustomEvent=previous;}
});
