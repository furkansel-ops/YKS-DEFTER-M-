...
function harness(){
  ...
  const host={
    ...
    __YKS_BACKUP__:{version:"2.0.0"},__YKS_PWA__:{build:"4.2.0-r1"},__YKS_V43_RUNTIME__:{ready:Promise.resolve({ok:true})},
...
test("kararlı sürüm denetimi açılış, v4.3 runtime, ekran, Dexie, kurtarma ve bulut yolunu birlikte doğrular",async()=>{
  ...
  try{
    ...
    assert.equal(result.ok,true);assert.equal(result.version,"4.2.0");assert.ok(result.checks.length>=28);assert.equal(result.checks.every(x=>x.ok),true);assert.equal(h.active,"home");assert.equal(h.applied,2);assert.match(h.output.textContent,/YKS_V4_RELEASE_OK/);assert.equal(api.latest(),result);
