...
function harness(){
  ...
  const host={
    __YKS_V4_BOOTSTRAP__:{version:"4.3.1",build:"4.3.1-r1",legacyCore:"4.1.0-r20",channel:"stable",legacyRuntime:true},
    __YKS_SERVICES__:{validate:()=>[]},__YKS_DOMAIN__:{validate:()=>[]},__YKS_SCREEN_RUNTIME__:{validate:()=>[]},__YKS_UI__:ui,__YKS_DATA__:data,
    __YKS_BACKUP__:{version:"2.0.0"},__YKS_PWA__:{build:"4.3.1-r1"},__YKS_V43_RUNTIME__:{ready:Promise.resolve({ok:true})},
...
test("kararlı sürüm denetimi açılış, v4.3 runtime, ekran, Dexie, kurtarma ve bulut yolunu birlikte doğrular",async()=>{
  ...
  try{
    ...
    assert.equal(result.ok,true);assert.equal(result.version,"4.3.1");assert.ok(result.checks.length>=28);assert.equal(result.checks.every(x=>x.ok),true);assert.equal(h.active,"home");assert.equal(h.applied,2);assert.match(h.output.textContent,/YKS_V4_RELEASE_OK/);assert.equal(api.latest(),result);
