...
test("kalıcı CI ana dalı denetler, tek Pages deploy kullanır ve eski tek-seferlik workflowlar kaldırılmıştır",()=>{
  ...
  assert.match(verify,/APP_BUILD=\"4\.1\.0-r20\"/);assert.match(verify,/yks-core-v4\.1\.0-r34/);assert.match(verify,/motivation-quotes-v1\.js/);assert.match(verify,/ui-polish-home-v2\.css/);assert.match(verify,/version\.json kararlı sürümle eşleşmiyor/);
});