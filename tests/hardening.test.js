...
test("kalıcı CI ana dalı denetler ve eski tek-seferlik workflowlar kaldırılmıştır",()=>{
  const ci=fs.readFileSync(path.join(root,".github/workflows/ci.yml"),"utf8"),verify=fs.readFileSync(path.join(root,"scripts/verify-dist.mjs"),"utf8");
  ...
  assert.match(verify,/APP_BUILD=\"4\.1\.0-r20\"/);assert.match(verify,/yks-core-v4\.1\.0-r20/);assert.match(verify,/version\.json kararlı sürümle eşleşmiyor/);
});
