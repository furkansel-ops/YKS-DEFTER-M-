...
test("kalıcı CI ana dalı denetler, tek Pages deploy kullanır ve eski tek-seferlik workflowlar kaldırılmıştır",()=>{
  ...
  assert.match(ci,/branches:\s*\n\s*- main/);assert.match(ci,/npm run build:assets/);assert.match(ci,/node scripts\/verify-release\.mjs/);assert.match(ci,/node scripts\/verify-v440-production\.mjs/);assert.doesNotMatch(ci,/npm run release:check/);
