...
if (typeof document !== "undefined") {
  void import("./biology-topic-map-v44.css").catch(()=>{});
  document.addEventListener("click",event=>{
    const target=event.target instanceof Element?event.target.closest<HTMLElement>("[data-atlas-map-step]"):null;
    if(!target)return;
    const step=Number(target.dataset.atlasMapStep);if(!Number.isInteger(step)||step<0)return;
    const panel=target.closest<HTMLElement>("#v320PanelAtlas");
    panel?.querySelector<HTMLButtonElement>(`.atlas-stage-roadmap [data-atlas-step="${step}"]`)?.click();
  });
}

export function atlasTopicMapV44(topic:AtlasTopic,currentStep:number):string {
  const plan=buildAtlasTopicMap(topic);
  const nodes=plan.nodes.map(node=>`<button type="button" class="atlas-topic-map-v44__node" data-atlas-map-step="${node.id}" aria-pressed="${node.id===currentStep}" ${node.id===currentStep?'aria-current="step"':""}><span class="atlas-topic-map-v44__number">${node.id+1}</span><span><b>${esc(node.label)}</b><small>${esc(node.detail)}</small></span></button>`).join("");
