...
export function installChemistryVisualsBridgeV44():ChemistryVisualsBridge {
  ...
  const hostVisible=()=>{const target=host();return !!target&&target.isConnected&&target.getClientRects().length>0;};
  ...
  const refresh=()=>{if(hostVisible()&&ensure()&&open)visuals?.mount(panel());};
  window.addEventListener("yks:more-after",event=>{
    const detail=(event as CustomEvent<RouteDetail>).detail;
    if(detail?.to==="lab"&&hostVisible()){ensure();if(open)visuals?.mount(panel());}
  });
  window.addEventListener("focus",()=>{if(open&&hostVisible())refresh();});
  if(hostVisible())queueMicrotask(ensure);
