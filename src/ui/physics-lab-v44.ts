...
export function createPhysicsLabV44():PhysicsLabApi {
  ...
  function visual(result:NonNullable<ReturnType<typeof evaluatePhysicsSimulation>>):string {
    const v=result.values,n=(key:string)=>read(v,key);
    if(result.id==="motion"){
      const x=clamp(250+n("dx")*5,45,555),dir=n("v")>=0?1:-1;
      return `<svg viewBox="0 0 600 220" role="img" aria-label="Hareket deney görünümü"><path d="M35 165H565" class="phy-line"/><path d="M70 140V188M530 140V188" class="phy-mark"/><circle cx="${x}" cy="145" r="25" class="phy-object"/><path d="M${x} 105h${dir*70}" class="phy-vector"/><text x="300" y="45">v = ${fmt(n("v"))} m/s · Δx = ${fmt(n("dx"))} m · F = ${fmt(n("force"))} N</text></svg>`;
    }
    if(result.id==="energy-momentum"){
      const k=clamp(Math.abs(n("kinetic"))/400,0,1),p=clamp(Math.abs(n("momentum"))/150,0,1),i=clamp(Math.abs(n("impulse"))/150,0,1);
    ...
    if(result.id==="circuits"){
      return `<svg viewBox="0 0 600 220" role="img" aria-label="Seri ve paralel devre karşılaştırması"><path d="M45 65H155V35H255V95H155V65M255 65H300" class="phy-circuit"/><rect x="168" y="24" width="72" height="22" rx="7"/><path d="M320 35H545V185H320Z" class="phy-circuit"/><rect x="375" y="24" width="72" height="22" rx="7"/><rect x="375" y="174" width="72" height="22" rx="7"/><text x="75" y="125">Seri: ${fmt(n("series"))} Ω</text><text x="355" y="125">Paralel: ${fmt(n("parallel"))} Ω</text><text x="75" y="150">I = ${fmt(n("iSeries"))} A</text><text x="355" y="150">I = ${fmt(n("iParallel"))} A</text></svg>`;
    }
    if(result.id==="fields"){
      const negative=result.state==="negative-charge";
      return `<svg viewBox="0 0 600 220" role="img" aria-label="Elektrik ve manyetik alan görünümü"><defs><marker id="phyArrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0 0L8 4L0 8Z"/></marker></defs><circle cx="300" cy="110" r="34" class="phy-object"/><text x="300" y="116" text-anchor="middle">${negative?"−":"+"}</text><path d="M95 110H250" class="phy-vector"/><path d="M350 110H505" class="phy-vector"/><text x="95" y="75">E</text><text x="390" y="75">Fe = ${fmt(n("electric"))}</text><text x="390" y="165">Fm = ${fmt(n("magnetic"))}</text></svg>`;
    }
    if(result.id==="optics"){
      const reflection=clamp(n("reflection"),0,80),refraction=n("refraction"),refr=refraction<0?80:clamp(refraction,0,80),toX=(angle:number,sign:number)=>300+sign*Math.sin(angle*Math.PI/180)*155;
      return `<svg viewBox="0 0 600 260" role="img" aria-label="Yansıma kırılma ve mercek görünümü"><path d="M45 130H555" class="phy-line"/><path d="M300 25V235" class="phy-normal"/><path d="M${toX(reflection,-1)} 25L300 130L${toX(reflection,1)} 25" class="phy-ray"/><path d="M300 130L${toX(refr,1)} 235" class="phy-ray ${result.state==="total-internal-reflection"?"is-hidden":""}"/><text x="40" y="35">i = r = ${fmt(reflection)}°</text><text x="365" y="225">${result.state==="total-internal-reflection"?"Tam yansıma":`kırılma ≈ ${fmt(refraction)}°`}</text></svg>`;
    }
    if(result.id==="waves"){
      const lambda=Math.max(.2,n("wavelength")),points:Array<string>=[];for(let px=0;px<=540;px+=12){const y=110-Math.sin(px/lambda*.11)*42;points.push(`${30+px},${y}`);}
      return `<svg viewBox="0 0 600 220" role="img" aria-label="Dalga ve girişim görünümü"><polyline points="${points.join(" ")}" class="phy-wave"/><path d="M60 175H540" class="phy-line"/><text x="60" y="45">λ = ${fmt(n("wavelength"))} m</text><text x="350" y="45">${result.state==="constructive"?"Yapıcı girişim":result.state==="destructive"?"Söndürücü girişim":"Kısmi girişim"}</text></svg>`;
    }
    if(result.id==="matter"){
      const pressure=n("pressure"),buoyancy=n("buoyancy"),heat=n("heat"),h=clamp(pressure/150000,0,1),b=clamp(buoyancy/800,0,1),thermal=clamp(heat/4000000,0,1);
      return `<svg viewBox="0 0 600 240" role="img" aria-label="Isı basınç ve kaldırma görünümü"><path d="M70 45V205H260V45" class="phy-vessel"/><rect x="71" y="${85-25*h}" width="188" height="${120+25*h}" class="phy-water"/><rect x="132" y="95" width="66" height="70" rx="8" class="phy-object"/><path d="M165 155V${155-85*b}" class="phy-vector"/><g class="phy-thermo"><rect x="390" y="45" width="34" height="150" rx="17"/><rect x="399" y="${185-thermal*125}" width="16" height="${thermal*125}" rx="8"/></g><text x="310" y="75">p = ${fmt(pressure)} Pa</text><text x="310" y="105">Fk = ${fmt(buoyancy)} N</text><text x="310" y="135">Q = ${fmt(heat)} J</text></svg>`;
    }
