...
function backdrop(scene: AtlasScene): string {
  switch (scene) {
    ...
    case "circulation": return ellipse(299,203,73,79,"var(--atlas-coral-soft)") + path("M299 131V278","var(--atlas-line)",3) + text(299,209,"KALP") + text(280,35,"AKCİĞER DEVRESİ") + text(295,409,"VÜCUT DEVRESİ");
...
export function atlasDiagram(topic: AtlasTopic, selected = 0, quiz = false, answer: number | null = null): string {
  ...
  const edges = links.map(([from,to,label]) => {
    ...
    return path(`M${x1+dx*31} ${y1+dy*31}L${x2-dx*34} ${y2-dy*34}`,"var(--atlas-line)",3,`marker-end="url(#${marker})"`) + (label && !quiz?text((x1+x2)/2,(y1+y2)/2-10,label):"");
  }).join("");
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 440" class="atlas-diagram" role="group" aria-label="${atlasEscape(topic.title)} etkileşimli şeması"><defs><marker id="${marker}" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0 0L10 5L0 10Z" fill="var(--atlas-line)"/></marker></defs><g aria-hidden="true">${backdrop(topic.scene)}${edges}</g>${nodes}</svg>`;
}
