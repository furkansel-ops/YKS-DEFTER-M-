...
function cardHtml(t:Teacher,favs:Set<string>):string{const favorite=favs.has(t.a),content=(t.t&&t.t.length?t.t:["konu","soru"]).slice(0,3),level=LEVEL_LABELS[t.l]||"Her seviye";return `<article class="teachers-v2-card" data-action="open" data-name="${esc(t.a)}" tabindex="0" role="button" aria-label="${esc(t.a)} ayrıntısını aç"><div class="teachers-v2-card-top"><div class="teachers-v2-avatar">${esc(initials(t.a))}</div><div><div class="teachers-v2-name">${esc(t.a)}</div><div class="teachers-v2-card-meta"><span class="teachers-v2-level" data-level="${esc(t.l)}">${esc(level)}</span>${t.d.slice(0,2).map(s=>`<span class="teachers-v2-subject">${esc(subjectLabel(s))}</span>`).join("")}</div></div><button class="teachers-v2-star ${favorite?"on":""}" type="button" data-action="favorite" data-name="${esc(t.a)}" aria-label="${favorite?"Favorilerden çıkar":"Favoriye ekle"}">★</button></div><p class="teachers-v2-note">${esc(t.n||"Bu hoca için kişisel kaynak notun burada görünecek.")}</p><div class="teachers-v2-tags">${content.map(x=>`<span class="teachers-v2-tag">${esc(TYPE_LABELS[x]||x)}</span>`).join("")}${t.own?'<span class="teachers-v2-tag">Senin eklediğin</span>':""}</div></article>`;}
...
function buildShell(root:HTMLElement):void{root.innerHTML=`<div class="teachers-v2-shell"><section class="teachers-v2-hero"><h2 class="teachers-v2-title">Hocalar ve videolar</h2><p class="teachers-v2-sub">Hocanı seç, serileri incele ve çalışacağın videoları programına ekle.</p><div class="teachers-v2-meta"><span id="teachersV2Count">0 hoca</span><span id="teachersV2FavCount">0 favori</span></div></section><div class="teachers-v2-toolbar"><label class="teachers-v2-search"><input id="teachersV2Search" type="search" autocomplete="off" placeholder="Hoca, ders veya açıklama ara…" aria-label="Hocalarda ara"></label><button id="teachersV2FavToggle" class="teachers-v2-fav-toggle" type="button">★ Sadece favoriler</button></div><div id="teachersV2SubjectRow" class="teachers-v2-filter-row" aria-label="Ders filtresi"></div><div id="teachersV2LevelRow" class="teachers-v2-filter-row" aria-label="Seviye filtresi"></div><div class="teachers-v2-summary"><span id="teachersV2ResultText"></span></div><div id="teachersV2Grid" class="teachers-v2-grid"></div></div>`;const search=root.querySelector("#teachersV2Search") as HTMLInputElement|null;search?.addEventListener("input",()=>{state.query=search.value;renderGrid();});root.querySelector("#teachersV2FavToggle")?.addEventListener("click",()=>{state.favOnly=!state.favOnly;savePrefs();renderAll();});root.addEventListener("click",handleRootClick);root.addEventListener("keydown",event=>{if(event.key!=="Enter"&&event.key!==" ")return;const target=event.target as HTMLElement|null,card=target?.closest<HTMLElement>(".teachers-v2-card[data-action='open']");if(card&&!target?.closest("button")){event.preventDefault();openDetail(card.dataset.name||"");}});}
...
function renderGrid():void{const grid=document.getElementById("teachersV2Grid");if(!grid)return;const teachers=currentTeachers(),favs=favoriteNames(),list=filteredTeachers(teachers,favs);grid.innerHTML=list.length?list.map(t=>cardHtml(t,favs)).join(""):'<div class="teachers-v2-empty"><b>Bu filtrede hoca bulunamadı.</b><br>Aramayı veya filtreleri değiştir.</div>';const result=document.getElementById("teachersV2ResultText");if(result)result.textContent=`${list.length} sonuç · ${teachers.length} toplam`;const count=document.getElementById("teachersV2Count");if(count)count.textContent=`${teachers.length} hoca`;const favCount=document.getElementById("teachersV2FavCount");if(favCount)favCount.textContent=`${favs.size} favori`;}
...
function handleRootClick(event:MouseEvent):void{const target=event.target as HTMLElement|null;if(!target)return;const actionNode=target.closest<HTMLElement>("[data-action]");if(!actionNode)return;const action=actionNode.dataset.action||"";if(action==="subject"){state.subject=actionNode.dataset.value||"";savePrefs();renderAll();return;}if(action==="level"){state.level=actionNode.dataset.value||"";savePrefs();renderAll();return;}if(action==="favorite"){event.stopPropagation();toggleFavorite(actionNode.dataset.name||"");return;}if(action==="open")openDetail(actionNode.dataset.name||"");}
...
function openDetail(name:string):void{const teacher=detailTeacher(name);if(!teacher)return;closeDetail();state.openName=name;const fav=favoriteNames().has(name),overlay=document.createElement("div");overlay.id=OVERLAY_ID;overlay.className="teachers-v2-overlay";overlay.setAttribute("role","dialog");overlay.setAttribute("aria-modal","true");overlay.setAttribute("aria-label",`${teacher.a} ayrıntısı`);overlay.innerHTML=`<section class="teachers-v2-detail"><div class="teachers-v2-detail-head"><strong>Hocalar ve videolar</strong><button class="teachers-v2-close" type="button" data-detail-action="close">Kapat</button></div><div class="teachers-v2-detail-body"><div class="teachers-v2-profile"><div class="teachers-v2-avatar">${esc(initials(teacher.a))}</div><div><h2>${esc(teacher.a)}</h2><div class="teachers-v2-card-meta"><span class="teachers-v2-level" data-level="${esc(teacher.l)}">${esc(LEVEL_LABELS[teacher.l]||"Her seviye")}</span>${teacher.d.map(s=>`<span class="teachers-v2-subject">${esc(subjectLabel(s))}</span>`).join("")}</div><p>${esc(teacher.n||"Bu hoca için kişisel kaynak notun bulunmuyor.")}</p></div><button class="teachers-v2-detail-star ${fav?"on":""}" type="button" data-detail-action="favorite" aria-label="Favori">★</button></div><section class="teachers-v2-section"><div class="teachers-v2-section-head"><h3>Video merkezi</h3><span>Oynatma listeleri, videolar ve çalışma programın</span></div><div class="teachers-v2-actions"><button class="teachers-v2-action" type="button" data-detail-action="youtube" data-kind="channel">Kanalı bul<small>Hoca + ders</small></button><button class="teachers-v2-action" type="button" data-detail-action="youtube" data-kind="tyt">TYT konu<small>Konu anlatımı</small></button><button class="teachers-v2-action" type="button" data-detail-action="youtube" data-kind="ayt">AYT konu<small>Konu anlatımı</small></button><button class="teachers-v2-action" type="button" data-detail-action="youtube" data-kind="soru">Soru çözümü<small>Pratik ve kamp</small></button><button class="teachers-v2-action" type="button" data-detail-action="youtube" data-kind="deneme">Deneme / branş<small>Çözüm videoları</small></button><button class="teachers-v2-action" type="button" data-detail-action="youtube" data-kind="">Tüm sonuçlar<small>${esc(teacher.d[0]||"YKS")}</small></button></div></section>${teacher.own&&Number.isFinite(teacher.id)?'<button class="teachers-v2-danger" type="button" data-detail-action="delete">Bu hocayı listemden sil</button>':""}</div></section>`;overlay.addEventListener("click",event=>{if(event.target===overlay){closeDetail();return;}const target=event.target as HTMLElement|null,action=target?.closest<HTMLElement>("[data-detail-action]");if(!action)return;const type=action.dataset.detailAction||"";if(type==="close"){closeDetail();return;}if(type==="favorite"){toggleFavorite(teacher.a);window.setTimeout(()=>{const on=favoriteNames().has(teacher.a);action.classList.toggle("on",on);action.setAttribute("aria-pressed",String(on));},80);return;}if(type==="youtube"){openYoutubeSearch(teacher,action.dataset.kind||"");return;}if(type==="delete"&&teacher.own&&Number.isFinite(teacher.id)&&window.confirm(`${teacher.a} listenden silinsin mi?`)){try{legacy.delTeacher?.(teacher.id as number);}catch{}closeDetail();window.setTimeout(renderAll,80);}});document.body.appendChild(overlay);document.body.classList.add("teachers-v2-open");}
