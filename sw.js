/* YKS Defterim — dayanıklı PWA katmanı | v3.1.0 */
const APP_VERSION="3.1.0";
const CACHE="yks-core-v3.1.0";
...
self.addEventListener("fetch",event=>{
  ...
  if(url.pathname.endsWith("/version.json")||url.pathname.endsWith("version.json")){
    event.respondWith(fetchWithTimeout(req,{cache:"no-store"},3500));return;
  }
  ...
  event.respondWith(caches.match(req).then(cached=>cached||fresh.catch(()=>null).then(r=>r||offlineResponse())));
});
