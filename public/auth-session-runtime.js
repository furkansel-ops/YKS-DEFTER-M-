...
function styles(){
  ...
  s.textContent=`.yas-account{margin-top:12px}.yas-account-row{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}.yas-account-meta{font-size:13px;color:var(--label-2,#667085);margin-top:4px}`;
...
function settingsCard(){
  ...
  card.innerHTML=`<div class="section-label">Hesap</div><div class="yas-account-row"><div><b data-account-title>Giriş yapılmadı</b><div class="yas-account-meta" data-account-meta>Bulut ve koçluk hesabı isteğe bağlıdır.</div></div><button class="btn ghost tiny" type="button" data-account-logout hidden>Çıkış yap</button></div>`;
...
function renderSettings(){
  ...
  if(!card)return;
  const title=card.querySelector("[data-account-title]"),meta=card.querySelector("[data-account-meta]"),logout=card.querySelector("[data-account-logout]");
  if(currentUser){
    title.textContent=currentUser.displayName||currentUser.email||"Hesap";
    const role=currentAccount?.role==="coach"?"Koç hesabı":"Öğrenci hesabı";
    meta.textContent=`${role}${currentUser.email?" · "+currentUser.email:""}`;
    logout.hidden=false;
  }else{
    title.textContent="Giriş yapılmadı";
    meta.textContent="Bulut ve koçluk hesabı isteğe bağlıdır.";
    logout.hidden=true;
  }
}
...
window.dispatchEvent(new CustomEvent("yks:auth-session-ready",{detail:{version:"1.4.0",publicRegistration:"student-only",sessionGate:"removed"}}));
