(function(){
  ...
  function runReleaseSelfTest(){
    ...
    const bootstrap=window.__YKS_V4_BOOTSTRAP__,release=window.__YKS_RELEASE__,meta=document.documentElement.dataset;
    try{
      add("version",typeof APP_VERSION!=="undefined"&&typeof APP_BUILD!=="undefined"&&APP_BUILD.startsWith(APP_VERSION+"-r")&&bootstrap?.legacyCore===APP_BUILD&&DATA_SCHEMA===21);
      add("modular-assets",!!document.querySelector('link[href="./app.css"],link[href*="assets/index-"][href$=".css"]')&&!!document.querySelector('script[src^="./app.js"]'));
      add("stability",typeof window.YKSSafeRender==="function"&&["persistRuntime","restoreRuntime","clearRuntime","updateOnlineBanner"].every(name=>typeof window.YKSStability?.[name]==="function"));
      ...
      add("v4-bootstrap",typeof bootstrap?.version==="string"&&bootstrap.version===release?.version&&bootstrap.version===meta.appVersion&&bootstrap.build===meta.v4ReleaseBuild&&bootstrap.build===window.__YKS_PWA__?.build&&bootstrap.channel==="stable"&&bootstrap.stableRelease===true);
      ...
      add("v4-release",typeof release?.version==="string"&&release.version===bootstrap?.version&&typeof release.run==="function");
