(function(root,factory){
  ...
  function wrongGroups(state){
    ...
    for(const w of (state&&state.wrongLog)||[]){
      ...
      let g=map.get(key);if(!g){g={key,subject,topic,total:0,entries:0,days:new Set(),last:"",kinds:{},examMarked:0};map.set(key,g);}
      g.total+=n;g.entries++;if(date){g.days.add(date);if(date>g.last)g.last=date;}g.kinds[kind]=(g.kinds[kind]||0)+n;if(w.deneme!=null)g.examMarked+=n;
    }
    return [...map.values()].map(g=>({...g,days:[...g.days]}));
  }
  ...
  function errorInsights(state,today){
    const groups=wrongGroups(state).map(g=>{const age=daysAgo(g.last,today),repeatDays=g.days.length;let score=g.total*3+repeatDays*10+(g.examMarked?8:0);if(age!=null&&age<=14)score+=12;if(repeatDays>=3)score+=12;return {...g,age,repeatDays,score};}).sort((a,b)=>b.score-a.score||b.total-a.total);
  ...
  function repeatRecommendations(state,signals,today){
    ...
    for(const s of signals||[]){
      ...
      const w=wrongMap.get(topicKey(subject,topic));const st=num(s.st),conf=num(s.conf),risk=num(s.riskScore||s.score),dueLate=num(s.dueLate),studyAge=s.studyLast?daysAgo(s.studyLast,today):null;
      let score=risk;
      const reasons=[];
      if(w){score+=Math.min(35,w.total*3)+Math.min(20,w.repeatDays*6);reasons.push(w.total+" yanlış"+(w.repeatDays>1?" · "+w.repeatDays+" farklı gün":""));}
      if(dueLate>0){score+=Math.min(28,12+dueLate);reasons.push(dueLate+" gün gecikmiş tekrar");}
      if(st>0&&st<3){score+=16;reasons.push("konu henüz pekiştirilmedi");}
      if(conf>0&&conf<=2){score+=18;reasons.push("güven "+conf+"/5");}
      if(st===3&&studyAge!=null&&studyAge>=21){score+=12;reasons.push("son çalışma "+studyAge+" gün önce");}
      for(const r of s.reasons||[])if(r&&!reasons.includes(r))reasons.push(String(r));
      if(score>=25)rows.push({exam:escText(s.exam),subject,topic,score:Math.round(score),reasons:reasons.slice(0,4),st,conf,dueLate,studyAge,wrong:w||null});
    }
    if(!rows.length){
      for(const w of errors.groups){if(w.score<25)continue;rows.push({exam:"",subject:w.subject,topic:w.topic,score:w.score,reasons:[w.total+" yanlış",w.repeatDays>1?w.repeatDays+" farklı gün":"yakın dönem yanlış kaydı"],st:0,conf:0,dueLate:0,studyAge:null,wrong:w});}
    }
    const dedup=new Map();for(const r of rows){const k=topicKey(r.subject,r.topic),old=dedup.get(k);if(!old||r.score>old.score)dedup.set(k,r);}
    return [...dedup.values()].sort((a,b)=>b.score-a.score).map(r=>({...r,severity:r.score>=75?"must":r.score>=50?"priority":"soon",label:r.score>=75?"TEKRAR ETMEN ŞART":r.score>=50?"ÖNCELİKLİ TEKRAR":"YAKINDA TEKRAR"}));
  }
  function topicHealth(signals,today){
    const out={notStarted:0,learning:0,consolidating:0,ready:0,repeat:0,total:0,rows:[]};
    for(const s of signals||[]){const st=num(s.st),risk=num(s.riskScore||s.score),late=num(s.dueLate),conf=num(s.conf),studyAge=s.studyLast?daysAgo(s.studyLast,today):null;let status="notStarted";
  ...
  function selfTest(){
    ...
    return {ok:rep[0]&&rep[0].severity==="must"&&err.repeated.length===1&&ex.delta===6&&health.repeat===1&&cmd.solved===80&&before===after,checks:{repeat:!!rep[0],readOnly:before===after,exam:ex.delta===6,error:err.repeated.length===1,topic:health.repeat===1}};
  }
  return {version:"3.0.0",KINDS,daysAgo,wrongGroups,errorInsights,examInsights,repeatRecommendations,topicHealth,commandCenter,snapshotSchedule,selfTest};
});
