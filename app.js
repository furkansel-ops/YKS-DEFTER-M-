...
function v4TopicService(){return window.__YKS_DOMAIN__&&window.__YKS_DOMAIN__.topics?window.__YKS_DOMAIN__.topics:null;}
function v4ExamTopicProgress(exam){
  const api=v4TopicService();if(api&&typeof api.examTopicProgress==="function")return api.examTopicProgress(S,ALL_SUBJECTS,exam);
  const rows=v26ExamTopicRows(exam),total=rows.length,steps=rows.reduce((sum,row)=>sum+row.t.st,0),done=rows.filter(row=>row.t.st===3).length,working=rows.filter(row=>row.t.st===1||row.t.st===2).length;
...
function v4TopicGoalSummary(){
  const api=v4TopicService();if(api&&typeof api.topicGoals==="function")return api.topicGoals(S,todayKey());
  const entries=deadlineList().map(x=>({key:x.key,exam:x.key.split("|")[0]||"",subject:x.subj,topic:x.topic,date:x.dl,daysLeft:x.left,done:x.done,status:x.done?"completed":x.left<0?"overdue":x.left===0?"today":"upcoming"}));
  return {total:entries.length,active:entries.filter(x=>!x.done).length,overdue:entries.filter(x=>x.status==="overdue").length,upcoming:entries.filter(x=>x.status==="today"||x.status==="upcoming").length,completed:entries.filter(x=>x.done).length,entries};
}
function v4UpcomingReviews(){
  const api=v4TopicService();if(api&&typeof api.upcomingReviewPlan==="function")return api.upcomingReviewPlan(S,todayKey(),REVIEW_GAPS,7);
  const out=[];Object.keys(S.topics||{}).forEach(key=>{const t=S.topics[key];if(t.st!==3||!t.ts)return;const gi=REVIEW_GAPS.findIndex((_,i)=>!(t.rev||[]).includes(i));if(gi<0)return;const gap=REVIEW_GAPS[gi],due=addDaysKey(t.ts,gap),daysLeft=diffKeys(todayKey(),due);if(daysLeft>7)return;const p=key.split("|");out.push({key,gi,gap,due,exam:p[0]||"",subj:p[1]||"",topic:p.slice(2).join("|"),late:Math.max(0,-daysLeft),status:daysLeft<0?"overdue":daysLeft===0?"today":"upcoming"})});return out.sort((a,b)=>a.due.localeCompare(b.due));
}
function v4OpenTopicGoalEditor(){openFold("hedef");const box=el("fh_hedef");if(box)box.scrollIntoView({behavior:"smooth",block:"start"});}
function v4RenderTopicGoals(){
  const w=el("v4TopicGoals");if(!w)return;const all=v4TopicGoalSummary(),entries=all.entries.filter(x=>x.exam===examTab),active=entries.filter(x=>!x.done),overdue=active.filter(x=>x.status==="overdue").length,completed=entries.filter(x=>x.done).length;
  let html='<div class="v4-topic-section-head"><div><span>'+esc(examTab)+' hedefleri</span><b>'+active.length+' aktif · '+overdue+' geciken · '+completed+' tamamlanan</b></div><button class="btn ghost tiny" type="button" onclick="v4OpenTopicGoalEditor()">Hedef ekle</button></div>';
  if(!active.length)html+='<div class="empty">Bu sınav türünde aktif konu hedefin yok. İstersen bir konuya bitiş tarihi verebilirsin.</div>';
  else html+='<div class="v4-topic-goal-list">'+active.slice(0,6).map(x=>{const when=x.status==="overdue"?Math.abs(x.daysLeft)+" gün gecikti":x.status==="today"?"Bugün":x.daysLeft+" gün kaldı";return '<button type="button" class="v4-topic-goal-row '+(x.status==="overdue"?'late':'')+'" onclick="openTopicDetail(\''+v26Arg(x.exam)+'\',\''+v26Arg(x.subject)+'\',\''+v26Arg(x.topic)+'\')"><span><small>'+esc(x.subject)+'</small><b>'+esc(x.topic)+'</b></span><i>'+esc(when)+'</i></button>';}).join("")+'</div>';
  html+='<p class="v4-topic-manual-note">Hedefler yalnız takip edilir; Programım tablosuna otomatik görev eklenmez.</p>';w.innerHTML=html;
}
...
function v26RenderOverview(ctx){
  const rows=v26ExamTopicRows(examTab),snap=v4ExamTopicProgress(examTab),total=snap.total,done=snap.done,work=snap.working,review=rows.filter(x=>ctx.due[x.key]).length,risk=rows.filter(x=>(ctx.priority[x.key]?.score||0)>=30).length,pct=snap.pct;
  const w=el("v26TopicOverview");if(w)w.innerHTML='<div class="v4-topic-progress-head"><div><small>'+examTab+' tamamlanma yüzdesi</small><strong>%'+pct+'</strong></div><span>'+snap.remainingSteps+' kademe kaldı</span></div><div class="v4-topic-progressbar"><i style="width:'+pct+'%"></i></div><div class="v26-topic-grid"><div class="v26-topic-kpi"><small>Tamamlanan</small><b>'+done+' / '+total+'</b></div><div class="v26-topic-kpi"><small>Çalışılıyor</small><b>'+work+'</b></div><div class="v26-topic-kpi"><small>Başlanmayan</small><b>'+snap.untouched+'</b></div><div class="v26-topic-kpi"><small>Tekrar / risk</small><b>'+review+' / '+risk+'</b></div></div><div class="v26-topic-summary">Yüzde, konu kademelerinden hesaplanır. Filtreler ve hedefler Programım tablosuna otomatik müdahale etmez.</div>';
  const defs=[["tfAll","Tümü",total],["tfWork","Çalışılıyor",work],["tfReview","Tekrar bekleyen",review],["tfRisk","Riskli",risk],["tfDone","Tamamlanan",done]];defs.forEach(([id,n,c],i)=>{const b=el(id);if(b){b.innerHTML=n+' <b>'+c+'</b>';b.classList.toggle("on",["all","work","review","risk","done"][i]===v26TopicFilter)}});v4RenderTopicGoals();
}
...
function renderReviewQueue(){
  const w=el("reviewBox");if(!w)return;const plan=v4UpcomingReviews(),q=plan.filter(x=>x.status!=="upcoming"),upcoming=plan.filter(x=>x.status==="upcoming");el("revCount").textContent=q.length;
  let html='<div class="v4-topic-section-head"><div><span>Tekrar planı</span><b>'+q.length+' şimdi gerekli · '+upcoming.length+' önümüzdeki 7 gün</b></div></div>';
  if(!q.length&&!upcoming.length){w.innerHTML=html+'<div class="empty">Şu an veya önümüzdeki 7 günde tekrar bekleyen konu yok.<br>Bir konuyu 3. kademeye aldığında 3., 7. ve 21. gün planı başlar.</div>';return}
  if(q.length)html+='<div class="v4-review-group"><small>Şimdi tekrar et</small>'+q.slice(0,30).map(r=>'<div class="revrow"><div><div class="rt">'+esc(r.subj)+' · '+esc(r.topic)+'</div><div class="rm">'+r.gap+'. gün tekrarı · '+parseKey(r.due).toLocaleDateString('tr-TR',{day:'numeric',month:'short'})+(r.status==="overdue"?' · '+r.late+' gün gecikmiş':' · bugün')+'</div></div><div class="rev-actions"><button class="btn ghost tiny" onclick="openTopicDetail(\''+v26Arg(r.exam)+'\',\''+v26Arg(r.subj)+'\',\''+v26Arg(r.topic)+'\')">Detay</button><button class="btn green tiny" onclick="doReview(\''+v26Arg(r.key)+'\','+r.gi+')">Yaptım</button></div></div>').join('')+'</div>';
  if(upcoming.length)html+='<div class="v4-review-group"><small>Sıradaki 7 gün</small>'+upcoming.slice(0,12).map(r=>'<div class="revrow"><div><div class="rt">'+esc(r.subj)+' · '+esc(r.topic)+'</div><div class="rm">'+r.gap+'. gün tekrarı · '+parseKey(r.due).toLocaleDateString('tr-TR',{day:'numeric',month:'short'})+'</div></div><button class="btn ghost tiny" onclick="openTopicDetail(\''+v26Arg(r.exam)+'\',\''+v26Arg(r.subj)+'\',\''+v26Arg(r.topic)+'\')">Detay</button></div>').join('')+'</div>';w.innerHTML=html;
}
...
function v4SaveTopicGoal(key){const input=el("v4TopicGoalDate"),date=input&&input.value;if(!/^\d{4}-\d{2}-\d{2}$/.test(date||"")){toast("Geçerli bir hedef tarihi seç");return false}const t=Object.assign({st:0,conf:0,ts:null,rev:[]},S.topics[key]);t.dl=date;S.topics[key]=t;save();renderDeadlines();renderSubjects();renderHome();renderTopicDetail();toast("Konu hedefi kaydedildi ✓");return true}
function v4ClearTopicGoal(key){clearDeadline(key);if(v26TopicOpen&&v26TopicOpen.key===key)renderTopicDetail();return true}
...
async function exportData(){
  if(window.__YKS_SESSION__&&!window.__YKS_SESSION__.canPersist()){toast("Yedek almak için hesabına giriş yap. Deneme çalışması kaydedilmez.");return false;}
  let text="";
  try{
...
function importData(inp){
  if(window.__YKS_SESSION__&&!window.__YKS_SESSION__.canPersist()){toast("Yedek yüklemek için hesabına giriş yap.");inp.value="";return false;}
...
function v4ApplyStoredJSON(json){
  try{
    const raw=safeJSONParse(json);if(!raw)throw new Error("Geçersiz JSON kaydı");
    if(Number(raw.v||1)>DATA_SCHEMA)throw new Error("Kayıt daha yeni veri şemasına ait");
    const next=normalize(Object.assign({},JSON.parse(JSON.stringify(DEF)),migrateState(raw)));
    const persisted=JSON.stringify(next);if(!persisted||persisted.length>MAX_REASONABLE_STATE_BYTES)throw new Error("Veri boyutu güvenli sınırı aştı");
    ...
    infraError("v4-apply-indexed",error);return {ok:false,message:String(error&&error.message||error||"Kayıt uygulanamadı")};
  }
}
...
function exportCSV(kind){
  if(window.__YKS_SESSION__&&!window.__YKS_SESSION__.canPersist()){toast("Çalışma kayıtlarını dışa aktarmak için hesabına giriş yap.");return false;}
  let txt,name;
...
function autoBackupRun(zorla){
  if(window.__YKS_SESSION__&&!window.__YKS_SESSION__.canPersist())return false;
  const bugun=todayKey();
...
function autoBackupRestore(gun){
  if(window.__YKS_SESSION__&&!window.__YKS_SESSION__.canPersist()){toast("Yedek yüklemek için hesabına giriş yap.");return false;}
...
/* v3.2.7 — 1000 kişi sözü; ek havuz public-domain quote corpusundan seçildi. */
...
function runBuiltInSelfTest(){
  ...
  const versionOk=typeof APP_VERSION==="string"&&APP_VERSION==="4.1.0"&&APP_CHANNEL==="Kararlı"&&DATA_SCHEMA===21;
...
function runV26SelfTest(){
  ...
  try{
    ...
    add("version",APP_VERSION==="4.1.0"&&V26_VERSION==="2.6.0"&&DATA_SCHEMA===21);add("study-no-double",st&&st.total===40);add("wrong",wr&&wr.total===5&&wr.examMarked===5);add("review",!!c.due[fkey]);add("risk",ri.score>=30);add("html",!!el("v26TopicOverview")&&!!el("v26TopicAttention")&&!!el("v26TopicModal")&&!!el("tfRisk"));
...
function runV251SelfTest(){
  ...
  try{
    ...
    add("version",APP_VERSION==="4.1.0"&&V251_VERSION==="2.5.1"&&DATA_SCHEMA===21);
...
const __v28Progress=renderProgress;renderProgress=function(){const r=__v28Progress();try{renderV28Progress()}catch(e){infraError('v28-progress',e)}try{if(typeof window.__YKS_RENDER_PROGRESS_DASHBOARD__==='function')window.__YKS_RENDER_PROGRESS_DASHBOARD__()}catch(e){infraError('v4-progress-dashboard',e)}return r;};
...
function v30GuestMode(){return window.__YKS_SESSION__?.mode==="guest"}
function v30CloudLabel(){if(v30GuestMode())return "Deneme · eşitleme kapalı";const state=el("cloudSyncBox")?.dataset?.state;return state==="synced"?"Hesabınla eşitlendi":state==="syncing"||state==="connecting"?"Eşitleniyor":state==="offline"?"Çevrimdışı · cihaz kaydı açık":state==="error"?"Bağlantı bekliyor":"Hesaba giriş gerekli"}
function v30RenderHome(){v30RenderQuick();const w=el("v30MoreStatus"),guest=v30GuestMode();if(w)w.innerHTML='<div class="v30-status-item"><b>'+(guest?'Kaydetmeden deneme':'Hesabın ve cihazın')+'</b><span>'+esc(v30CloudLabel())+'</span></div><div class="v30-status-item"><b>'+(guest?'Yedekleme kapalı':'Son yedek · '+esc(v30BackupLabel()))+'</b><span>'+(guest?'Yenilenince veya kapanınca silinir':esc(v30StorageLabel())+' yerel veri')+'</span></div><div class="v30-status-item"><b>v'+esc(APP_VERSION)+'</b><span>'+esc(APP_CHANNEL)+' · şema '+DATA_SCHEMA+'</span></div>';v30RenderAbout();v30RenderDataSyncSummary()}
function v30RenderDataSyncSummary(){const w=el("v30DataSyncSummary");if(!w)return;const guest=v30GuestMode();w.innerHTML='<div class="v30-sync-line"><span>Ana kayıt</span><b>'+(guest?'Geçici bellek · kaydedilmez':'Bu cihaz · Dexie + yerel ayna')+'</b></div><div class="v30-sync-line"><span>Hesap / bulut</span><b>'+esc(v30CloudLabel())+'</b></div><div class="v30-sync-line"><span>JSON yedek</span><b>'+(guest?'Hesaba giriş gerekli':esc(v30BackupLabel()))+'</b></div><div class="v30-sync-line"><span>'+(guest?'Geçici veri':'Yerel veri')+'</span><b>'+esc(v30StorageLabel())+'</b></div>'}
...
function runV31SelfTest(){
  ...
  try{
    add("version",APP_VERSION==="4.1.0"&&V31_VERSION==="3.1.3"&&DATA_SCHEMA===21);
...
(function(){
  ...
  function buildDeneme(){
    ...
    /* v3.2.7 Deneme Analiz Merkezi kendi tam-genişlik düzenini yönetir. */
  ...
  function rebuildProgress(){
    const root=by('progress'),modern=by('v4ProgressCoreGrid');
    if(root&&modern)return !!modern.querySelector('#progressCompare')&&!!modern.querySelector('#progressNet');
    const score=by('v28Score');if(!root||!score)return false;
  ...
  function runV314SelfTest(){
    ...
    try{
      const a=repair();add('version',APP_VERSION==='4.1.0'&&LAYOUT_VERSION==='3.1.4');add('layout-health',a.ok);
...
/* ---- legacy-block-7 ---- */
/* ==================================================================
   YKS Defterim v3.2.7 — Deneme Analiz Merkezi
...
/* Genel self-test zincirine v3.2.7'i ekle. */
...
/* ---- legacy-block-8 ---- */
/* ==================================================================
   YKS Defterim v3.2.7 — Odak Bahçesi
...
/* ---- legacy-block-10 ---- */
/* ==================================================================
   YKS Defterim v3.2.7 — Büyük Günün Sözü + Başarı & Ödül Merkezi
   Tüm ödüller mevcut kayıtların salt-okuma türevleridir; uygulama şeması 20'dir.
   ================================================================== */
const V319_VERSION="3.2.7";
...
function runV319SelfTest(){
  ...
  try{
    add("version",APP_VERSION==="4.1.0"&&V319_VERSION==="3.2.7"&&DATA_SCHEMA===21);
