...
if(localVersion.version!=="4.3.1"||localVersion.build!=="4.3.1-r1"||localVersion.schema!==21)throw new Error(`Beklenmeyen v4.3.1 kimliği: ${JSON.stringify(localVersion)}`);
...
const checks={
  ...
  pwa:sw.includes(`const APP_VERSION="${localVersion.version}"`)&&sw.includes(`const APP_BUILD="${localVersion.build}"`)&&sw.includes(`const CACHE="yks-core-v${localVersion.build}"`)&&sw.includes('"yks-core-v4.3.0-r1"')&&sw.includes('"yks-core-v4.2.0-r1"')
};
const failed=Object.entries(checks).filter(([,ok])=>!ok).map(([name])=>name);
if(failed.length)throw new Error(`Kararlı sürüm kontrolleri başarısız: ${failed.join(", ")}`);
console.log(`YKS Defterim ${pkg.version} kararlı sürümü doğrulandı: ${Object.keys(checks).length} paket kontrolü + izole v4.3 özellik kapısı`);
