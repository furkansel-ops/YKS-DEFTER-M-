...
export function verifyLiveAssets({index,bundle,legacyApp}){
  ...
  if(!index.includes('./app.js?v=4.0.0-final')||!index.includes('./modules/release-selftest.js?v=4.0.0-final'))throw new Error("Canlı sayfada beklenen çalışma zamanı dosyaları bağlı değil");
  ...
  for(let attempt=1;attempt<=attempts;attempt++){
    try{
      ...
      const [bundle,legacyApp]=await Promise.all([
        ...
        fetchText(new URL("app.js?v=4.0.0-final",base),token)
      ]);
